...
class HashMap:
    ...
    def put_modded(self, arr_to_put_in, capacity, key, value):
        ...
        if linked_list_at_index.length() == 0:
            ...
            key_in = linked_list_at_index.contains(key)  # Grab a pointer to node that contains key if exists
    ...
    def resize_table(self, new_capacity: int) -> None:
        ...
        for _ in range(self.capacity):
            ...
            for node in linked_list:
                new_size_counter += self.put_modded(new_buckets, new_capacity, node.key, node.value)
